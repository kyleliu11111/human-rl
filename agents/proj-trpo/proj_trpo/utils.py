...
def vectorized_rollout(env, agent, max_timesteps_per_episode, timesteps_per_batch, predictor, render=False):
    paths = []
    timesteps_elapsed = 0
    first = True
    while timesteps_elapsed < timesteps_per_batch:
        ...
        for _ in range(max_timesteps_per_episode):
            timesteps_elapsed += 1
            obs.append(ob)
            action, action_dist, ob, info = agent.act(ob)
            actions.append(action)
            # action_dists_mu.append(action_dist)
            action_dists_mu.append(info.get("action_dist", []))
...
def slice_2d(x, inds0, inds1):
    ...
    # print("asdf 2", x_flat.shape)
    # x_flat = tf.reshape(x, [-1])
    tmp = tf.gather(x_flat, inds0 * ncols + inds1)
    return tmp
